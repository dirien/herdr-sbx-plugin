...
| Deciding a mapping is busy | `bridgeIsRunning(entry)` (`src/lifecycle.mjs`; the bridge records its pid in `acknowledgeBridge`, clears it in `releaseBridge`, and the check also verifies the process runs `bridge.mjs --pane-id <pane>` so a recycled pid never counts) before Herdr's agent detection; destructive actions and `reconnect` go through `refuseWhileAgentRuns` before the popup, and `lifecycle.destroy` re-reads the mapping and re-checks both right before every `sbx rm` |
