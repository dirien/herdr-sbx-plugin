...
| Adding a config key | Add to the `PluginConfig` typedef, `CONFIG_DEFAULTS` and `validateConfig` (`src/config.mjs`), document in README table, test in `test/config.test.mjs` |
