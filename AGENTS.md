...
| Deciding a mapping is busy | `bridgeIsRunning(entry)` (`src/lifecycle.mjs`; the bridge records its pid in `acknowledgeBridge`) before Herdr's agent detection; destructive actions and `reconnect` go through `refuseWhileAgentRuns` |
